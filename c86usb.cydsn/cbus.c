...
*/
static void CBUS_SetAddr(uint8_t slot, uint16_t addr)
{
	uint8_t ah = (uint8_t)(addr>>8);
	switch(slot){
	case 0:
		CBusAddrRegU_Write((ah&0x80) ? 0x00 : 0xf0 );  //    0000 1111
		CBusAddrRegH_Write(ah);
		break;
	case 1:
		CBusAddrRegU_Write((ah&0x80) ? 0x80 : 0x70 ); // A23 1000 0111
		CBusAddrRegH_Write(ah^0x80); // bit7反転
		break;
	case 2:
		CBusAddrRegU_Write((ah&0x80) ? 0x40 : 0xb0 ); // A22 0100 1011
		CBusAddrRegH_Write(ah^0x80); // bit7反転
		break;
	case 3:
		CBusAddrRegU_Write((ah&0x80) ? 0x20 : 0xd0 ); // A21 0010 1101
		CBusAddrRegH_Write(ah^0x80); // bit7反転
		break;
	}
