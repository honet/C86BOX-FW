...
// Generated on 09/24/2014 at 02:49
// Component: CBUS_CTRL
module CBUS_CTRL (
	output  BHE,
	output  DOUT_EN,
...
localparam [4:0] STATE_WRITE_WAIT_IORDY              = {2'b10, 3'd0}; // 2,0
localparam [4:0] STATE_WRITE_RELOAD_HI_PULSE_COUNTER = {2'b00, 3'd5}; // 0,5  HIパルス幅設定値をD1->A1へロード
localparam [4:0] STATE_WRITE_WAIT_HI_PULSE           = {2'b00, 3'd7}; // 0,7  A1ダウンカウントしてHIパルス幅生成

localparam [4:0] STATE_READ_RELOAD_LO_PULSE_COUNTER = {2'b01, 3'd4};  // 1,4
localparam [4:0] STATE_READ_WAIT_LO_PULSE           = {2'b01, 3'd6};  // 1,6
localparam [4:0] STATE_READ_WAIT_IORDY              = {2'b11, 3'd0};  // 3,0
localparam [4:0] STATE_READ_RELOAD_HI_PULSE_COUNTER = {2'b01, 3'd5};  // 1,5
localparam [4:0] STATE_READ_WAIT_HI_PULSE           = {2'b01, 3'd7};  // 1,7
...
begin
    if(reset) begin
        ...
        begin
            if (z0_detect)
                state <= (IORDY) ? STATE_READ_RELOAD_HI_PULSE_COUNTER : STATE_READ_WAIT_IORDY;
            else
                state <= STATE_READ_WAIT_LO_PULSE;
        end
