...
// Generated on 09/16/2016 at 03:09
// Component: CBusDIn
module CBusDIn (
	input       CLKIN,
	input       reset,
...
wire data_ready;     /* provide data ready status to system bus       */
wire data_full;      /* Data FIFO full status to system bus          */
...
localparam [4:0] STATE_LOAD_CMD                      = {2'b01, 3'd1}; // 1,1  CMD_FIFOからA0へロード
localparam [4:0] STATE_DECODE_CMD                    = {2'b01, 3'd0}; // 1,0  A0をpoに出力し、それをデコード

localparam [4:0] STATE_READ_RELOAD_LO_PULSE_COUNTER = {2'b00, 3'd4};  // 1,4  LOWパルス幅設定値をD0->A0へロード
localparam [4:0] STATE_READ_WAIT_LO_PULSE           = {2'b00, 3'd6};  // 1,6  A0ダウンカウントしてLOWパルス幅生成
localparam [4:0] STATE_READ_WAIT_IORDY              = {2'b10, 3'd0};  // 3,0 IORDYがアサートされている間待つ
localparam [4:0] STATE_READ_FETCH_DATA              = {2'b00, 3'd2};  // 0,2  DINをpinからロードしてDATA_FIFOに出力
localparam [4:0] STATE_READ_RELOAD_HI_PULSE_COUNTER = {2'b00, 3'd5};  // 1,5  HIパルス幅設定値をD1->A1へロード
localparam [4:0] STATE_READ_WAIT_HI_PULSE           = {2'b00, 3'd7};  // 1,7  A1ダウンカウントしてHI状態保持
  ...
  begin
    if (reset) begin
      ...
      case(state)
        ...
        STATE_IDLE:
          begin
            //state <= (cmd_ready) ? STATE_LOAD_CMD : STATE_IDLE;
            if (IORDY && cmd_ready)
        ...
        // ---------------------------
        // READ
        STATE_READ_RELOAD_LO_PULSE_COUNTER:
          state <= STATE_READ_WAIT_LO_PULSE;
        STATE_READ_WAIT_LO_PULSE:
          begin
            if (z0_detect)
              //state <= (IORDY) ? STATE_READ_FETCH_DATA : STATE_READ_WAIT_IORDY;
              if (IORDY)
                state <= STATE_READ_FETCH_DATA;
              else
                state <= STATE_READ_WAIT_IORDY;
            else
              state <= STATE_READ_WAIT_LO_PULSE;
          end

        STATE_READ_WAIT_IORDY:
          begin
            //state <= (IORDY) ? STATE_READ_FETCH_DATA : STATE_READ_WAIT_IORDY;
            if (IORDY)
              state <= STATE_READ_FETCH_DATA;
        ...
        STATE_READ_FETCH_DATA:
          state <= STATE_READ_RELOAD_HI_PULSE_COUNTER;
        STATE_READ_RELOAD_HI_PULSE_COUNTER:
          state <= STATE_READ_WAIT_HI_PULSE;
        STATE_READ_WAIT_HI_PULSE:
          begin
            //state <= (z1_detect) ? STATE_IDLE : STATE_READ_WAIT_HI_PULSE;
            if (z1_detect)
              state <= STATE_IDLE;
          end
...
assign status[3'd1] = data_ready;     /* transparent */
assign status[2] = 1'd0;
...
always @(posedge op_clk)
  begin
    NIOR <= ~(state == STATE_READ_RELOAD_LO_PULSE_COUNTER | state == STATE_READ_WAIT_LO_PULSE | state == STATE_READ_WAIT_IORDY);
  end
...
wire [7:0] din_msb;
wire [7:0] din_lsb;
assign din_msb = (cmd_is_byte_access) ? 8'd0: DIN_H;
assign din_lsb = (cmd_is_byte_access && is_odd) ? DIN_H : DIN_L;

// データバス読取タイミング
wire data_load;
assign data_load = (state == STATE_READ_FETCH_DATA);
...
.cy_dpconfig(
{
    ...
    `CS_CMP_SEL_CFGA, /*CFGRAM2:    READ_DATA */
...
(
    ...
    /* output */         .f1_bus_stat(data_ready),
    /* output */         .f1_blk_stat(data_full),
